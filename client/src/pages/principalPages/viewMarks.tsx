...
function PrincipalViewMarks() {
    ...
    const sortOptions = ["None", "Student Name", "Subject Name", "Class", "Marks Obtained"];
    ...
    if (classFilter !== "All") {
        processedMarks = processedMarks.filter(
            (mark) => mark.studentId?.class === classFilter
        )
    };
    ...
    if (sortBy === "Class") {
        processedMarks.sort((a, b) => {
            const numA = parseInt(a.studentId?.class ?? "0", 10);
            const numB = parseInt(b.studentId?.class ?? "0", 10);
            return numA - numB;
        });
    }
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [sortBy, orderBy, classFilter, search, examType, limit]);
